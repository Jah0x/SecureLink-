...
- Поле тарифов `price_cents` используется во всех слоях вместо `price`, добавлен флаг `is_demo` и эндпоинт `/api/demo`; админка и публичный интерфейс поддерживают демо-тарифы и выдают их пользователю только один раз.
- Исправлен unit-тест тарифов: `is_demo` в `InMemoryPlanRepo.create` теперь необязателен и дефолтно равен `false`, что устраняет ошибки TypeScript при сборке.
- Интегрирован сервис подписок: добавлен subsClient, серверные маршруты /api/subs/*, страница управления подпиской для пользователей и вкладка админки, обновлены .env.example и README.
